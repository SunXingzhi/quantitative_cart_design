...
if __name__ == "__main__":
    ...
    zero2w = thread_handler.raspberry(
        pi_config.SERIAL_PATH, pi_config.BAUD_RATE, pi_config.TIMEOUT_TIME
    )
    # 开启与MCU的串口通信线程（读数据）
    pi_receive_MCU_thread = threading.Thread(
        target=thread_handler.pi_mcu_communication_thread,
        args=(zero2w,),
        name="pi_mcu_communicator",
    )
    pi_receive_MCU_thread.start()
    # print("[√] pi received_data thread open successfully")
    # 开启MCU数据解析线程
    pi_pasrsing_MCU_thread = threading.Thread(
        target=thread_handler.parse_mcu_data_thread,
        args=(zero2w,),
        name="pi_MCU_data_parser",
    )
    ...
    try:
        ...
        while True:
            ...
            if thread_handler.GUI_receive_status == 1 and pi_receive_MCU_thread != None:
                ...
                query_type = data_handler.parse_client_command(
                    thread_handler.received_data
                )
                if query_type == "points":
                    ...
                    coordinates_list = list(
                        map(float, (thread_handler.received_data[3:-1]).split("/"))
                    )
                ...
                if query_type == "Nstart":
                    ...
                    if points_number == 0:
                        print("[×] Invalid coordinates number")
                    # 开启导航线程
                    navigation_single_thread = threading.Thread(
                        target=thread_handler.navigation_thread,
                        args=(zero2w, coordinates_list, points_number),
                        name="nevigation_thread_handler",
                    )
            ...
            # 解析电机速度
            motors_speed = car_motion_handler.parsed_motors_speed(
                joystick_data["v_cx"], joystick_data["omega"]
            )
            # 解析舵机角度

            motors_angle = car_motion_handler.parsed_motors_angle(
                joystick_data["lt"], joystick_data["rt"]
            )
            ...
            if send_counter == 3:
                ...
                print(
                    f"\t\t\t\t\t\tmotor:{usart_motor_cmd},servo:{usart_servo_cmd}",
                    end="\r",
                )  # debug
            if send_counter == 4:
