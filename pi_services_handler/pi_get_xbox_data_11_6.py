...
# 创建与GUI通讯线程
GUI_communication_thread	= threading.Thread(target=socket_connect_thread, args=('172.16.100.46', 65432))
GUI_send_thread			= threading.Thread(target=socket_send_thread, args=('172.16.100.46', 65432))
...
def get_motor_data_by_status(car_status, xbox_data):
	match car_status:
		...
		case 1:		# 默认待机：只保留 lx, ly，其余清零
			for field_name, _ in xbox_data._fields_:
				if field_name not in ['lx', 'ly', 'rx', 'ry']:
					setattr(xbox_data, field_name, 0)
			# print(xbox_data)
			return xbox_data
...
def convert_joystick_data(xbox_data):
	...
	# 转换比例
	lx = int(xbox_data.lx / pi_config.JOY_AXIS_MAX * pi_config.TURNING_RADIUS_MAX)
	ly = -int(xbox_data.ly / pi_config.JOY_AXIS_MAX * 100)  # Y轴反向（推前为正）
	rx = int(xbox_data.rx)
	ry = int(xbox_data.ry)
...
if __name__ == "__main__":
	...
	try:
		...
		while True:
			...
			# 解析舵机角度
			motors_angle = parsed_motors_angle(joystick_data['rx'], joystick_data['ry'])
			print(f"map_data:{joystick_data['rx']}motors_angle:{motors_angle}")
			...
			# 发送到串口
			# print(f"data:{usart_cmd}" + usart_cmd_2, end='\r')	# debug
			try:
				out_put_serial.write(usart_motor_cmd.encode("utf-8"))
				out_put_serial.write(usart_servo_cmd.encode("utf-8"))	
			except Exception as e:
				print(f"串口发送失败: {e}")

			# 将数据放到共享变量，便于与GUI通讯的线程获取数据
			GUI_communicator_information = usart_motor_cmd + usart_servo_cmd + '@n/106.42714000753222/29.81969819780613*'  # 添加定位信息
